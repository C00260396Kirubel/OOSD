...
{
   return ( hour < 10 ? "0" : "" ) + hour +   //just outputs the hour actually does nothing really
          ( minute < 10 ? "0" : "" ) + minute;
}
...
public String toString()
{
   return ( ( hour == 12 || hour == 0 ) ? 12 : hour % 12 ) +
          ":" + ( minute < 10 ? "0" : "" ) + minute +       //if minute is less than 10 it outputs zero first for e.g 09 instead of just 9
          ":" + ( second < 10 ? "0" : "" ) + second +       //same above in seconds
          ( hour < 12 ? " AM" : " PM" );		            //checks if hour is less than 12 if so its am else pm
}

// Tick the time by one second
public void tick()
{
   setSecond( second + 1 );     //add a second to second

   if ( second == 0 )           // if second=0  increment the minute .... the hour inside of minute... then just set the hour
      incrementMinute();
}
